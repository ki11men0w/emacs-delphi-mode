...
;; To enter OpascalEx mode when you find a Opascal source file, one must override
;; the auto-mode-alist to associate Opascal with .pas (and .dpr and .dpk)
;; files. Emacs, by default, will otherwise enter Pascal mode. E.g.
;;
;; (autoload 'opascal-mode "opascal")
;; (setq auto-mode-alist
;;       (cons '("\\.\\(pas\\|dpr\\|dpk\\)$" . opascal-mode) auto-mode-alist))

;; To get keyword, comment, and string literal coloring, be sure that font-lock
;; is running. One can manually do M-x font-lock-mode in a Opascal buffer, or
;; one can put in .emacs:
;;
;; (add-hook 'opascal-mode-hook 'turn-on-font-lock)
...
;; when opascal-mode is autoloaded.

;; When you have entered Opascal mode, you may get more info by pressing
;; C-h m.

;; This opascal mode implementation is fairly tolerant of syntax errors, relying
...
(defcustom opascal-indent-level 3
  "*Indentation of Opascal statements with respect to containing block. E.g.
...
(defcustom opascal-tab-always-indents t
  "*Non-nil means TAB in Opascal mode should always reindent the current line,
regardless of where in the line point is when the TAB command is used."
  :type 'boolean
  :group 'opascal)

(defcustom opascal-newline-always-indents t
  "*Non-nil means NEWLINE in Opascal mode should always reindent the current
...
(defmacro opascal-save-match-data (&rest forms)
...
non-opascal buffer. Set to nil in a opascal buffer.  To override, just do:
 (let ((opascal-ignore-changes t)) ...)")

(defun opascal-after-change (change-start change-end old-length)
...
(defun opascal-newline ()
  "Terminate the current line with a newline and indent the next, unless
`opascal-newline-always-indents' is nil, in which case no reindenting occurs."
  (interactive)
  ;; Remove trailing spaces
  (delete-horizontal-space)
  (newline)
  (when opascal-newline-always-indents
...
(defvar opascal-debug-mode-map
  (let ((kmap (make-sparse-keymap)))
    (mapc #'(lambda (binding) (define-key kmap (car binding) (cadr binding)))
	  '(("n" opascal-debug-goto-next-token)
...
(defvar opascal-mode-map
  (let ((kmap (make-sparse-keymap)))
    (mapc #'(lambda (binding) (define-key kmap (car binding) (cadr binding)))
	  (list '("\r" opascal-newline)
		'("\t" opascal-tab)
		'("\177" backward-delete-char-untabify)
;;              '("\C-cd" opascal-find-current-def)
...
(defconst opascal-mode-syntax-table (make-syntax-table)
  "Opascal mode's syntax table. It is just a standard syntax table.
...
;;;###autoload
(defun opascal-mode (&optional skip-initial-parsing)
  "Major mode for editing Opascal code. \\<opascal-mode-map>
\\[opascal-tab]\t- Indents the current line (or region, if Transient Mark mode
\t  is enabled and the region is active) of Opascal code.
...
Customization:
 ...
 `opascal-tab-always-indents'          (default t)
    Non-nil means TAB in Opascal mode should always reindent the current line,
    regardless of where in the line point is when the TAB command is used.
 `opascal-newline-always-indents'      (default t)
    Non-nil means NEWLINE in Opascal mode should always reindent the current
...
Turning on Opascal mode calls the value of the variable opascal-mode-hook with
no args, if that value is non-nil."
  (interactive)
  (kill-all-local-variables)
  (use-local-map opascal-mode-map)
  (setq major-mode 'opascal-mode)
  (setq mode-name "OpascalEx")

  (setq local-abbrev-table opascal-mode-abbrev-table)
  (set-syntax-table opascal-mode-syntax-table)
  ...
  (mapc #'(lambda (var)
	...
	(list '(indent-line-function opascal-indent-line)
	      '(comment-indent-function opascal-indent-line)
	      '(case-fold-search t)
	      '(opascal-progress-last-reported-point nil)
	      '(opascal-ignore-changes nil)
	      (list 'font-lock-defaults opascal-font-lock-defaults)))
  ...
  (run-mode-hooks 'opascal-mode-hook))

;; Associate ourselves with pascal/delphi files files.
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.\\(pas\\|dpr\\|dpk\\)$" . opascal-mode))
